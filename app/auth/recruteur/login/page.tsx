...
export default function RecruiterLoginPage() {
    ...
    try {
      ...
      if (res.data) {
        ...
        if (res.data.user.emailVerified === false) {
          ...
          window.location.href = `/auth/verify-email?email=${encodeURIComponent(data.email)}&type=recruteur`;
          return;
        }
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: "100vh", fontFamily: '"Inter Tight", system-ui, sans-serif' }}>
      ...
      <AuthForm type="login" userType="recruteur" onSubmit={handleSubmit} isLoading={isLoading} />
    </div>
  );
}
